...
class OmniPR2Teleop:
    def __init__(self, arm, ff):
        ...
        self.enabled_r = False
	self.enabled_l = False
        ...
        if arm == "l" or arm == "b":
            ...
            rospy.Subscriber('omni2_button', PhantomButtonEvent, self.omni_safety_lock_cb_l)
            if ff == True:
        ...
        if arm == "r" or arm == "b":
            self.right_controller = ControlPR2Arm(
                                       ...
                                       tfbroadcast=self.tfbroadcast,
                                       tflistener=self.tflistener)
            rospy.Subscriber('omni1_button', PhantomButtonEvent, self.omni_safety_lock_cb_r)
        ...
        self.set_state(False, "b")


    def omni_safety_lock_cb_r(self, msg):
        if msg.grey_button == 1 and msg.white_button == 1:
            self.set_state(not self.enabled_r, "r")

    def omni_safety_lock_cb_l(self, msg):
        if msg.grey_button == 1 and msg.white_button == 1:
            self.set_state(not self.enabled_l, "l")

    def set_state(self, s, arm):
	...
	"""
        if self.enabled:
			...
			print "Setting Both Omni On"
                	cont.set_control(True)
	    elif arm == "r":
		for cont in self.controller_list:
			if cont == self.right_controller:
				print "Setting Right Omni On"
				cont.set_control(True)
	    elif arm == "l":
		for cont in self.controller_list:
			if cont == self.left_controller:
				print "Setting Left Omni On"
				cont.set_control(True)
            ...
            if arm == "b":
                for cont in self.controller_list:
			print "Setting both Omnis off"
                        cont.set_control(False)
            elif arm == "r":
                for cont in self.controller_list:
                        if cont == self.right_controller:
				print "Setting Right Omni off"
        	                cont.set_control(False)
            elif arm == "l":
                for cont in self.controller_list:
                        if cont == self.left_controller:
				print "Setting Left Omni off"
                                cont.set_control(False)
	...
	if arm == "b":
		self.enabled_r = s
		self.enabled_l = s
		self.left_controller.set_control(self.enabled_l)
		self.right_controller.set_control(self.enabled_r)
		#for f in self.ff_list:
			#f.set_enable(s)
	elif arm == "r":
		self.enabled_r = s
		self.right_controller.set_control(self.enabled_r)
		#for f in self.ff_list:
			#if f == self.right_feedback:
				#f.set_enable(self.enabled_r)
	elif arm == "l":
		self.enabled_l = s
		self.left_controller.set_control(self.enabled_l)
		#for f in self.ff_list:
			#if f == self.left_feedback:
				#f.set_enable(self.enabled_l)
	for f in self.ff_list:
        	f.set_enable(False)
