...
class DetectTowel(SuccessFailureState):
    ...
    def execute(self,userdata):
	robot_head = bp.Head()
	robot_head.set_pan_tilt(0.0,0.8)	
        rospy.sleep(1)
...
class ExecuteFoldSQP(SuccessFailureState):
	...
	def execute_trajectory(self, traj, pt_tl, pt_tr, pt_bl):
		...
		left_gripper = bp.Gripper('l')
                right_gripper = bp.Gripper('r')
	
		left_gripper.open(block=False)
		right_gripper.open(block=False)
		...
		dt = TIME/17.0	
		#left_arm.goto_cart(left_arm.get_pvec(),block=False,T=5)
		#right_arm.goto_cart(right_arm.get_pvec(),block=True,T=5)
		#left_arm.goto_cart(traj[1][0],block=False,T=dt)
		#right_arm.goto_cart(traj[2][0],block=True,T=dt)

		"""
		for i in range(14):
			left_arm.goto_cart(traj[1][i],block=False)
			right_arm.goto_cart(traj[2][i],block=True)
		"""
		
		traj[1].insert(1,left_arm.backed_up_grasp(traj[1][1]))
		traj[2].insert(1,right_arm.backed_up_grasp(traj[2][1]))
		traj[1].insert(6,left_arm.backed_up_grasp(traj[1][6]))
		traj[2].insert(11,right_arm.backed_up_grasp(traj[2][11]))
		#traj[1].insert(0,left_arm.get_pvec())
		#traj[2].insert(0,right_arm.get_pvec())
		opt_left = left_arm.follow_cart_traj(traj[1],T=TIME,execute=False)
		opt_right = right_arm.follow_cart_traj(traj[2],T=TIME,execute=False)
		...
		for i in range(1,17):
			...
			if i != 9:
			#if i != 11:	
				#left_arm.follow_timed_joint_traj([opt_left[i-1-offset_l].positions, opt_left[i-offset_l].positions],[opt_left[i-1-offset_l].velocities, opt_left[i-offset_l].velocities],[0,dt],block=False)
			#else:
			#	offset_l += 1
			#if i != 6:
				#right_arm.follow_timed_joint_traj([opt_right[i-1-offset_r].positions, opt_right[i-offset_r].positions],[opt_right[i-1-offset_r].velocities, opt_right[i-offset_r].velocities],[0,dt],block=True)	
			#else:
			#	offset_r += 1
				
				#if i != 9:
				#if i != 11:
                        	left_arm.follow_timed_joint_traj([opt_left[i-offset_l].positions],[opt_left[i-offset_l].velocities],[0],block=False)
				#else:
				#	offset_l += 1
                        	#if i != 6:
				right_arm.follow_timed_joint_traj([opt_right[i-offset_r].positions],[opt_right[i-offset_r].velocities],[0],block=True)
				#else:
				#	offset_r += 1    
		...
		#for j in range(1,16):
		#	opt_left[j].velocities = zeros
		#	opt_right[j].velocities = zeros
		def compute_times(l,r):
			n_l = len(l)
			n_r = len(r)
			times = [0.0]
			for i in range(1,n_l):
				time_l = 5 * mag_diff(l[i],l[i-1])		
				time_r = 5 * mag_diff(r[i],r[i-1])
				times.append(max(time_l,time_r))
				times[i] = dt

			return times

		def mag_diff(pvec1,pvec2):
			return sqrt((pvec1[0] - pvec2[0])**2 + (pvec1[1]-pvec2[1])**2 + (pvec1[2]-pvec2[2])**2)


		times = compute_times(traj[1],traj[2])
	
			
		i = 1
                left_arm.follow_timed_joint_traj([opt_left[i].positions, opt_left[i+1].positions],[opt_left[i].velocities, opt_left[i+1].velocities],[times[i], times[i]+times[i+1]],block=False)
                right_arm.follow_timed_joint_traj([opt_right[i].positions, opt_right[i+1].positions],[opt_right[i].velocities,opt_right[i+1].velocities],[times[i], times[i+1]],block=True)
                left_gripper.close()
                right_gripper.close(block=True)

                i = 3
                left_arm.follow_timed_joint_traj([opt_left[i].positions, opt_left[i+1].positions],[opt_left[i].velocities,opt_left[i+1].velocities],[times[i], times[i]+times[i+1]],block=False)
                right_arm.follow_timed_joint_traj([opt_right[i].positions,opt_right[i+1].positions],[opt_right[i].velocities,opt_right[i+1].velocities],[times[i], times[i]+times[i+1]],block=True)
                
		#i = 4
		#left_arm.follow_timed_joint_traj([opt_left[i].positions],[opt_left[i].velocities],[dt],block=False)
                #right_arm.follow_timed_joint_traj([opt_right[i].positions],[opt_right[i].velocities],[dt],block=True)

		left_gripper.open()
                right_gripper.open(block=True)
	
                i = 5
                left_arm.follow_timed_joint_traj([opt_left[i].positions, opt_left[i+1].positions,opt_left[i+2].positions],[opt_left[i].velocities, opt_left[i+1].velocities,opt_left[i+2].velocities],[times[i], times[i]+times[i+1], times[i]+times[i+1]+times[i+2]],block=False)
                right_arm.follow_timed_joint_traj([opt_right[i].positions, opt_right[i+1].positions],[opt_right[i].velocities,opt_right[i+1].velocities],[times[i], times[i]+times[i+1]+times[i+2]],block=True)
                left_gripper.close(block=True)
               
		i = 8 
                left_arm.follow_timed_joint_traj([opt_left[i].positions, opt_left[i+2].positions],[opt_left[i].velocities, opt_left[i+2].velocities],[times[i], times[i]+times[i+1]],block=False)
                right_arm.follow_timed_joint_traj([opt_right[i-1].positions, opt_right[i].positions],[opt_right[i-1].velocities,opt_right[i].velocities],[times[i], times[i]+times[i+1]],block=True)
                left_gripper.open(block=True)

                i = 11
                left_arm.follow_timed_joint_traj([opt_left[i].positions, opt_left[i+1].positions],[opt_left[i].velocities, opt_left[i+1].velocities],[times[i], times[i]+times[i+1]+times[i+2]],block=False)
                right_arm.follow_timed_joint_traj([opt_right[i-1].positions, opt_right[i].positions, opt_right[i+1].positions],[opt_right[i-1].velocities,opt_right[i].velocities,opt_right[i+1].velocities],[times[i], times[i]+times[i+1], times[i]+times[i+1]+times[i+2]],block=True)
                right_gripper.close(block=True)
		...
		state_space_size = 14
                left_arm = bp.Arm('l')
                right_arm = bp.Arm('r')
		print left_arm.get_pvec()
		print right_arm.get_pvec()		
		#print pt_bl.header.frame_id
		#print pt_tr.header.frame_id
		
		pvecs_l = []
		for i in range(1,11):
			#new_pvec_l = left_arm.get_pvec()
			#new_pvec_l[0:3] += 0.01*i
			pvecs_l.append(new_pvec_l) #trajectory[1]
		
               	pvecs_r = []
                for i in range(1,11):
                        #new_pvec_r = right_arm.get_pvec()
                        #new_pvec_r[0:3] += 0.01*i
                        pvecs_r.append(new_pvec_r) #trajectory[1]
		
		
		pvecs_l = trajectory[1]
		pvecs_r = trajectory[2]

		#left_arm.goto_posture("up")
		#right_arm.goto_posture("up")
		left_arm.follow_cart_traj(pvecs_l,T=10,block=False)
		right_arm.follow_cart_traj(pvecs_r,T=10,block=True)		
		...
		right_gripper = bp.Gripper('r')
                ...
                left_joints = left_arm.follow_cart_traj(pvecs_l,T=40,block=False)
                right_joints = right_arm.follow_cart_traj(pvecs_r,T=40,block=True)  

		"""
		
		#ctrl_name = "l_arm_controller"
		#joint_command_l = rospy.Publisher('%s/command' % ctrl_name, tm.JointTrajectory)
		#ctrl_name = "r_arm_controller"
		#joint_command_r = rospy.Publisher('%s/command' % ctrl_name, tm.JointTrajectory)
	
		#for i in range(14):
		#	print "publishing a point"
		#	joint_command_r.publish(right_joints[i])
		#	joint_command_l.publish(left_joints[i])
		#	rospy.sleep(4)	
			

		"""

		 def goto_cart_for_grasp(self,pvec,dist=.05):
        	"first go to a point behind the target, then move fwd to target"
        	pvec_back = pvec.copy()
        	pvec_back[0:3] -= bgeom.quat2vec(pvec[3:7]) * dist
        	self.goto_cart(pvec_back)
        	self.goto_cart(pvec)
		
	   	
	
		cat_left_joints = copy.deepcopy(left_joints)
		cat_left_joints.points = [cat_left_joints.points[0]]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = [cat_right_joints.points[0]]	
		joint_command_l.publish(cat_left_joints)
		joint_command_r.publish(cat_right_joints)	
		...
		right_gripper.close()
                ...
                cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[1:3]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[1:3]
		joint_command_l.publish(cat_left_joints)
                joint_command_r.publish(cat_right_joints)
		...
		cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[3:5]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[3:5]
		...
		left_gripper.close()
                ...
                cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[5:7]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[5:7]
		...
		left_gripper.open()
                ...
                cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[7:9]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[7:9]
		...
		right_gripper.close()
                ...
                cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[9:11]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[9:11]
		...
		right_gripper.open()
                ...
                cat_left_joints = copy.deepcopy(left_joints)
                cat_left_joints.points = cat_left_joints.points[11:13]
                cat_right_joints = copy.deepcopy(right_joints)
                cat_right_joints.points = cat_right_joints.points[11:13]
