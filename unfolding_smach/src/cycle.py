...
from pr2_simple_motions_srvs.srv import MoveBase, RotateBase, MoveBaseToPose
import math
...
class SetTablePositions(NestedStateMachine):
    def __init__(self, listener, title=None, transitions=None):
        ...
        self.add('Save_Dropoff_Position',SavePosition('dropoff', listener),{SUCCESS:SUCCESS,FAILURE:FAILURE})
...
def go_to(location, listener):
    ...
    try:
        set_pose = rospy.ServiceProxy('set_pose', MoveBaseToPose)
        resp1 = set_pose(location)    
    except rospy.ServiceException, e:
        print "Service call failed: %s"%e
...
class DropOff(NestedStateMachine):
    def __init__(self, listener, title=None, transitions=None):
        ...
        self.add('Let_Go',LetGo(), {SUCCESS:SUCCESS, FAILURE: FAILURE})

class LetGo(SuccessFailureState):
...
def main(args):
    rospy.init_node("location_cycle_demo_node")

    listener = tf.TransformListener()

    sm = OuterStateMachine(DEFAULT_OUTCOMES)
    with sm:
         OuterStateMachine.add('Set_Table_Positions',SetTablePositions(listener),{SUCCESS:'Cycle',FAILURE:FAILURE})
